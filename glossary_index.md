...
### 接受-拒绝法
假设$p(x)$不可以直接抽样。找一个可以直接抽样的分布，称为建议分布(proposal distribution)。假设$q(x)$是建议分布的概率密度函数，并且有$q(x)$的$c$倍一定大于等于$p(x)$，其中$c>0$。按照$q(x)$进行抽样，假设得到的结果是$x^*$，再按照$\frac{p(x^*)}{cq(x^*)}$的比例决定是否接受$x^*$。
这里可以配个图。
