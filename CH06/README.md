...
- 书中没有直接给出LR的损失函数，在[CH12](../CH12/README.md)中有提到LR的损失函数是逻辑斯谛损失函数. 如果采用损失函数上界的那类分析方法，定义$\cal{Y}=\{+1,-1\}$， 有$\log_2(1+\exp(yf(x)))=\log_2(1+\exp(y(w\cdot x)))$
...
一般模型的**特征**是关于x的函数, 最大熵模型中的特征函数, 是关于x和y的函数. 注意理解$f(x)$与$f(x, y)$的区别. 关于特征函数可以参考[条件随机场](../CH11/README.md)中例11.1关于特征部分的内容增强理解.
...
1. 大多数算法感觉都是在刷权重, 考虑哪些量可以用, 哪些方法可以让权重刷的更合理, 哪些方法能刷的更快.
