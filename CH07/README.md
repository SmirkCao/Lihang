...
- 支持向量机学习策略是**间隔最大化**，可形式化为一个求解凸二次规划的问题，也等价于正则化的合页损失函数的最小化问题。
...
$$
**在超平面$w\cdot x+b=0$确定的情况下**，函数间隔可以表示分类**预测(强调一下)**的**正确性**及**确信度**。书上在说明函数间隔的缺陷的时候，说了`在选择分离超平面时，`。

所以这一段的意思是说，在**预测**的时候，函数间隔可以表示分类的正确性及确信度。在**训练**的过程中，需要选择分离超平面，这时等比例的改变$w, b$得到的超平面实际上是一样的。
...
好，到这里，讲到了软间隔最大化和硬间隔最大化。软间隔最大化的提出，是因为有些数据集存在outlier，不满足线性可分的条件，这时候原来约束最优化问题中的不等式约束不能完全满足，对不满足的这些点支付一个惩罚，就引入了软间隔最大化。这里$C>0, \xi >=0$ 其中$\xi >0$的情况对应了outlier。
